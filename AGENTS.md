...
- `scripts/prepare-photos.py` が原稿 → `public/images/products/<folder>/{1..n}.webp`・`scenes/*.webp`・`texture/weave-*.webp`・`data/image-sizes.json` を作る。
  ...
  - **原稿はスクリーンショット**で、上端（横位置）か左端（縦位置）に 29 / 66px の黒帯が乗っている。`trim_bars()` が落とす。
  - **縦位置の PNG には古い EXIF Orientation=8 が残っている**（画素はもう正しい向き）。タグを連れたまま rembg に渡すと 90° 倒れたマスクが返ってくるので、`load()` で画素だけにしてから使う（実際に踏んだ）。`バンブー小11` だけは画素そのものが倒れているので `rotate: 90`。
