...
Blog（`/blog`）の記事は microCMS から読む。商品カタログ（`data/products.ts`）は
...
| `MICROCMS_BLOG_ENDPOINT` | – | API 名。既定 `blogs`。すでに別名で作ってあるならここで指定する |
| `MICROCMS_WEBHOOK_SECRET` | – | Webhook の署名検証。未設定だと `/api/revalidate` は 503 |

**二つの必須変数が無い環境では、`data/blog.ts` の seed 記事が表示される。**
鍵の無いローカルやプレビューでもビルドと表示が通るようにするため。API が落ちた
ときも同じ経路で seed に落ち、理由は server log（`[microcms]`）に出る。

## 2. API スキーマ

**リスト形式**の API を一つ作る。API ID は `blogs`。
コンテンツ ID がそのまま URL になる（`/blog/<コンテンツID>`）ので、
...
| `imageRole` | セレクト | | `blog` / `material-macro` / `lifestyle` / `process`（既定 `blog`） |
...
本文の組みは `app/globals.css` の `.blog-prose`。見出し・引用・箇条書き・画像・
...
`revalidateTag("blog")` を呼ぶ。署名が合わなければ 401 で何もしない。

動作確認:

```bash
BODY='{"service":"miroku","api":"blogs"}'
...
- 本文中から特定の記事へ送る導線（ヒーローの「On the material」、商品ページの
  「Care note」、トップの Material セクション）は `blogHref()` を通していて、
  slug を焼き込んでいない。指定 slug → 同じ topic の記事 → 最新 → 一覧、の順に
  落ちるので、CMS 側で記事を消してもリンク切れにならない。ただし **`topic` を
  `Materials` / `Care` で入れておくと**、意図した記事に当たる。
- `data/blog.ts` は型とフォールバックの種だけ。記事を足すのは microCMS の管理画面。
- 旧 URL `/journal/*` は `next.config.ts` で `/blog/*` へ恒久リダイレクト。
